...
// BuscarUsuarios busca todos os usuários com uso de um filtro salvo no banco de dados
...
// BuscarSeguidoresUsuario traz todos os seguidores de um usuario
func BuscarSeguidoresUsuario(w http.ResponseWriter, r *http.Request) {
	parametros := mux.Vars(r)
	usuarioID, erro := strconv.ParseUint(parametros["usuarioId"], 10, 64)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
...
// BuscarSeguindoUsuario traz todos os usuarios que um determinado usuario está seguindo
func BuscarSeguindoUsuario(w http.ResponseWriter, r *http.Request) {
	parametros := mux.Vars(r)
	usuarioID, erro := strconv.ParseUint(parametros["usuarioId"], 10, 64)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
...
// AtualizarSenhaUsuario permite alterar a senha de um usuario
func AtualizarSenhaUsuario(w http.ResponseWriter, r *http.Request) {
	...
	corpoRequesicao, erro := io.ReadAll(r.Body)
	...
	if erro = seguranca.VerificarSenha(senhaSalvaNoBanco, senha.Atual); erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, errors.New("a senha atual não condiz com a senha está no banco"))
		return
	}
